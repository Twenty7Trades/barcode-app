...
function addSessionFromJSON(sessionData) {
  const config = loadConfig();
  ...
  const session = {
    ...
    files: Array.isArray(sessionData.files) ? sessionData.files : [],
    blockers: Array.isArray(sessionData.blockers) ? sessionData.blockers : [],
...
function addSession() {
  ...
  // Check if JSON data is provided via stdin or command line
  const jsonInput = process.argv[3];
  if (jsonInput && jsonInput.startsWith('{')) {
    try {
      const sessionData = JSON.parse(jsonInput);
      return addSessionFromJSON(sessionData);
    } catch (e) {
  ...
  // Check if reading from stdin (non-interactive mode)
  if (!process.stdin.isTTY) {
    ...
    process.stdin.on('end', () => {
      try {
        ...
        addSessionFromJSON(sessionData);
      } catch (e) {
        console.error('❌ Invalid JSON input:', e.message);
        process.exit(1);
      }
...
switch (command) {
  ...
  default:
    ...
    console.log('  node notes-agent/notes-agent.js session       Add a session summary (interactive)');
    console.log('  node notes-agent/notes-agent.js session \'{"title":"..."}\'  Add session from JSON');
